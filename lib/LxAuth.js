...
//               logging.syslog.error('%s! getting user from db: %j', error, name);
    ...
    self.updateLastActivity = function (token, callback) {
        ...
        if (lxHelpers.isNumber(config.WEB_TOKEN.inactivityTime)) {
            updateStatement.$set.expires_last_activity = new Date(Date.now() + config.WEB_TOKEN.inactivityTime);
        }

        tokensRepo.update({_id: token._id}, updateStatement, callback);
    };
        ...
        tokensRepo.find({access_id: webToken.accessId}).toArray(function (err, success) {
            ...
            self.updateLastActivity(token, function () {
                ...
                var user = {
                    ...
                    rolesAsObjects: token.user_acl.roles_as_objects
                };
        ...
        async.auto({
            ...
            getTokenData: ['getUserAcl', function (next, results) {
                ...
                var tokenData = {
                    ...
                    ex: new Date(Date.now() + config.WEB_TOKEN.maxLifeTime * 1000)
                };
