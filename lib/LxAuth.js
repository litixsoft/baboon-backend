...
module.exports = function (options) {
    ...
    function addRoleRights(roles, allRoles, result) {
        ...
        _.forEach(roles, function (roleId) {
    ...
    self.getRightsByRole = function (role, callback) {
        ...
        //noinspection Eslint
        rolesRepo.findOne({ name: role }, function (errRoles, resRoles) {
            if (errRoles) {
                return callback(errRoles);
            }

            if (resRoles) {
                //noinspection Eslint
                rightsRepo.find({ _id: { $in: resRoles.rights } }).toArray(function (errRights, resRights) {
                    if (errRights) {
                        return callback(errRights);
                    }

                    if (resRights) {
                        _.forEach(resRights, function (right) {
    ...
    self.getUserRights = function (user, allRoles, allGroups, additionalRoles, resourceRights) {
        ...
        if (lxHelpers.isArray(allGroups) && !lxHelpers.isEmpty(user.groups)) {
            _.forEach(user.groups, function (groupId) {
                // get group
        ...
        if (lxHelpers.isArray(additionalRoles)) {
            _.forEach(additionalRoles, function (role) {
        ...
        // add user rights
        _.forEach(user.rights || [], function (right) {
    ...
    self.getUserAcl = function (user, allRights, allRoles, allGroups, additionalRoles, resourceRights) {
        ...
        _.forEach(userRights, function (userRight) {
            // get right
    ...
    self.getUser = function (user, callback) {
        async.auto({
            ...
            getUserGroups: ['getUser', function (results, next) {
            ...
            getUserRoles: ['getUser', 'getUserGroups', function (results, next) {
                if (!results.getUser) {
                    next(null, []);
                } else {
            ...
            getUserResourceRights: ['getUser', 'getUserGroups', 'getUserRoles', function (results, next) {
                if (!results.getUser) {
                    next(null, []);
                } else {
                    var roles = results.getUser.roles || [],
            ...
            normalizeResourceRights: ['getUserResourceRights', function (results, next) {
                ...
                async.each(resourceRights, function (resourceRight, innerCallback) {
                    if (resourceRight.right_id) {
                        ...
                        rolesRepo.findOne({ _id: resourceRight.role_id }, function (error, role) {
                            ...
                            _.forEach(role.rights, function (rightId) {
            ...
            getUserAcl: ['getAllRights', 'getUserRoles', 'normalizeResourceRights', function (results, next) {
    ...
    self.refreshRightsInDb = function (callback) {
        ...
        async.auto({
            ...
            processRights: ['getRoutesFromRoutes', 'getPublicFunctionsFromControllers', function (results, next) {
                ...
                function addRightToRoles(rightRoles, rightId) {
                    _.forEach(rightRoles, function (role) {
                        role = role.trim();
            ...
            processRoles: ['processRights', function (results, next) {
     ...
     *
     * @param {String} route The route which was called.
     * @param {String|Object} verb The http method of the request.
     * @param {Object} acl The acl object of the current user.
     * @returns {boolean}
     */
    self.checkAccessToRoute = function (route, verb, acl) {
        ...
        if (lxHelpers.isObject(verb)) {
            // extract verb from methods object in express, take the first one
            // http://expressjs.com/4x/api.html#req.route
            verb = Object.keys(verb)[0];
        }

        return Boolean(acl[getRouteNameWithVerb(route, verb)]);
    };
    ...
    self.checkToken = function (tokenData, next) {
        ...
        tokensRepo.findOne({ access_id: webToken.accessId }, function (err, token) {
            // if not found -> error
            if (err || !token) {
    ...
    self.createToken = function (name, user, callback) {
        async.auto({
            ...
            getTokenData: ['getUserAcl', function (results, next) {
                // access object
                var tokenData = {
            ...
            saveAccessData: ['getTokenData', function (results, next) {
                tokensRepo.insertOne(results.getTokenData, next);
            }],
            createWebToken: ['saveAccessData', function (results, next) {
     ...
     * @param next
     */
    self.removeToken = function (tokenData, next) {
        var webToken;
        ...
        tokensRepo.deleteMany({ access_id: webToken.accessId }, next);
    };
